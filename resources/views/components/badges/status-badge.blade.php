...
@php
    // 1. Ekstrak nilai string dari Enum jika $status berupa BackedEnum (VehicleStatus)
    $statusValue = $status instanceof \BackedEnum ? $status->value : (string) $status;
    $statusString = strtolower($statusValue);

    // 2. Tentukan class Tailwind berdasarkan status
    ...
    // 3. Tentukan teks label (Gunakan method label() dari Enum jika ada, atau capital awal jika string biasa)
    $displayLabel = $status instanceof \App\Enums\VehicleStatus ? $status->label() : ucfirst($statusString);
@endphp
