...
@section('title', 'Tambah Kendaraan Baru - RentRide')
@section('page_title', 'Tambah Kendaraan Baru')

@section('content')
<div class="max-w-5xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Form Tambah Armada</h1>
            <p class="text-xs text-slate-500 mt-1">Lengkapi data di bawah ini untuk menambahkan unit kendaraan baru.</p>
        </div>
        ...
        <form action="{{ route('admin.vehicles.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="Contoh: Avanza Veloz / NMAX Turbo" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:bg-white transition @error('name') border-red-500 @enderror">
                    @error('name')
                        <p class="text-xs text-red-500 mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="plate_number" id="plate_number" value="{{ old('plate_number') }}" placeholder="Contoh: N 1234 ABC" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:bg-white transition @error('plate_number') border-red-500 @enderror">
                    @error('plate_number')
                        <p class="text-xs text-red-500 mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div>
                    ...
                    <input type="number" name="year" id="year" value="{{ old('year', date('Y')) }}" placeholder="2024" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:bg-white transition @error('year') border-red-500 @enderror">
                    @error('year')
                        <p class="text-xs text-red-500 mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="color" id="color" value="{{ old('color') }}" placeholder="Hitam Metalik" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:bg-white transition @error('color') border-red-500 @enderror">
                    @error('color')
                        <p class="text-xs text-red-500 mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="price_per_day" id="price_per_day" value="{{ old('price_per_day') }}" placeholder="350000" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:bg-white transition @error('price_per_day') border-red-500 @enderror">
                    @error('price_per_day')
                        <p class="text-xs text-red-500 mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                <label for="thumbnail" class="block text-xs font-bold text-slate-700 uppercase mb-2">Foto Utama Kendaraan (Thumbnail)</label>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" placeholder="Masukkan fasilitas, kondisi unit, atau informasi penting lainnya..." class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:bg-white transition">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-xs text-red-500 mt-1.5">{{ $message }}</p>
                @enderror
            </div>
